...
### Security model привязки (production-grade, сделано 2026-08-18)

Привязка по коду построена как «подтверждение» вместо мгновенного линка:

1. **Явное подтверждение**: `/link ABC123` не привязывает сразу — бот
   отвечает «Вы привязываете Telegram @username к аккаунту …?» с кнопками
   «Да, привязать» / «Отмена». Привязка выполняется только после нажатия
   «Да» (код при этом одноразовый — расходуется в момент подтверждения).
   Подтвердить может только тот, кто отправил `/link` (защита в групповых
   чатах).
2. **Отвязка**: в Профиле → Telegram кнопка «Отвязать» — двухшаговая
   (первый клик «взводит», второй подтверждает), исключает случайный разрыв.
3. **Активные сессии**: Профиль показывает Telegram @username, дату
   подключения и **последнюю активность** (`lastActiveAt` обновляется при
   каждом обращении к боту и каждом входе через Telegram).
4. **Webhook replay protection**: каждый апдейт записывается в
   `telegramSeenUpdates` по `update_id` до обработки; повторная доставка
   того же апдейта (Telegram ретраит при сетевых сбоях) пропускается —
   мутация не выполнится дважды, записи еды/воды не задвоятся. Строки
   старше суток подчищаются автоматически.
