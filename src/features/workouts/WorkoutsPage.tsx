...
import { planChangeSummary, weekStart, workoutArt } from "./lib/workoutUtils";
...
export default function WorkoutsPage() {
  ...
  const handleGenerate = async (silent = false) => {
    ...
    try {
      ...
      track("workout_plan_generated", {
        sessions: template.sessionsPerWeek ?? 0,
        goal: profile.fitnessGoal,
      });
  ...
  // Автопересборка плана: если данные профиля изменились, план пересобирается сам.
  useEffect(() => {
  ...
  const tonnageData = useMemo(() => {
    ...
    for (const log of logs ?? []) {
      ...
      for (const ex of log.exercises) {
        // Единый источник фактических подходов: setDetails, если заполнены.
        const sets = ex.setDetails && ex.setDetails.length > 0 ? ex.setDetails : [];
        if (sets.length > 0) {
          for (const set of sets) {
            if (set.weightKg > 0) sum += set.weightKg * set.reps;
          }
        } else if (ex.weightKg > 0) {
          sum += ex.weightKg * ex.reps * ex.sets;
        }
      }
      if (sum === 0) continue;
  ...
  const prs = useMemo(() => {
    ...
    for (const log of logs ?? []) {
      for (const ex of log.exercises) {
        const sets = ex.setDetails && ex.setDetails.length > 0 ? ex.setDetails : null;
        const topWeight = sets
          ? Math.max(...sets.map((s) => s.weightKg), 0)
          : ex.weightKg;
        if (topWeight <= 0) continue;
        const cur = best.get(ex.name);
        if (!cur || topWeight > cur.weightKg) {
          best.set(ex.name, {
            weightKg: topWeight,
            date: log.date,
            sets: sets?.length ?? ex.sets,
            reps: sets ? sets[0].reps : ex.reps,
          });
        }
      }
    }
  ...
  const handleSaveTraining = async (
    ...
    try {
      await logWorkout({
        ...
        exercises,
        effort,
        idempotencyKey: newIdempotencyKey("workout-log"),
      });
  ...
  return (
    <div className="relative isolate mx-auto max-w-4xl space-y-10">
      ...
      {/* Сводная карточка плана: сплит, частота, цикл и профиль */}
        ...
        <section className="space-y-5">
          ...
          {visibleDays
            ...
            .map((day, idx) => {
              ...
              // Длительность сессии считаем из подходов и отдыха.
                        ...
                        return (
                          <li key={tipKey}>
                            <div className="flex items-center justify-between gap-3 rounded-lg bg-surface-container-low px-3 py-2.5">
                              ...
                              {/* Подходы × повторы + отдых с иконкой */}
