/**
 * Чистые форматирующие хелперы страницы «Тренировки» (вынесены из
 * Workouts.tsx): арт-карта фокусов, начало недели, сводка изменений плана.
 */
...
export function workoutArt(focus: string): { icon: LucideIcon; gradient: string } {
  return WORKOUT_ART[focus] ?? { icon: Dumbbell, gradient: "to-primary-container/50" };
}
